...
public class Agent_SEIR : MonoBehaviour
{
    [Header("Movement")] 
    ...
    public IEnumerable<Agent_SEIR> NearbyAgents()
    {
        return NearbyAgents(InfectionRadius);
    }

    public IEnumerable<Agent_SEIR> NearbyAgents(float radius)
    {
        int n = Physics2D.OverlapCircleNonAlloc(transform.position, radius, Colliders, AgentMask);
        return Colliders
            .Take(n)
            .Select(collider => collider.GetComponent<Agent_SEIR>());
    }
